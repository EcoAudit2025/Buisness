...
try:
    ...
    # Add a section for database stats with error handling
    st.sidebar.title("Database Stats")
    try:
        all_users = db.get_all_users()
        public_users = db.get_public_users()
        total_usage_records = len(db.get_utility_history(limit=1000))
        
        st.sidebar.markdown(f"""
        - **{len(all_users) if all_users else 0}** total users
        - **{len(public_users) if public_users else 0}** public profiles
        - **{len(popular_materials) if popular_materials else 0}** materials tracked
        - **{total_usage_records}** usage records
        """)
        
        # Show environmental class distribution
        if all_users:
            class_counts = {'A': 0, 'B': 0, 'C': 0, 'Unclassified': 0}
            for user in all_users:
                env_class = user.environmental_class or 'Unclassified'
                if env_class in class_counts:
                    class_counts[env_class] += 1
                else:
                    class_counts['Unclassified'] += 1
            
            st.sidebar.markdown("**Environmental Classes:**")
            for class_name, count in class_counts.items():
                if count > 0:
                    st.sidebar.markdown(f"- Class {class_name}: {count}")
    
    except Exception as db_error:
        st.sidebar.error(f"Database connection issue: {str(db_error)[:50]}...")
        
except Exception as e:
    st.sidebar.error("Loading database statistics...")
...
if page == "User Profile":
        ...
        with tab2:
            ...
            if st.button("Create Account", key="create_button"):
                if new_username:
                    ...
                    existing_user = db.get_user(new_username)
                    if existing_user:
    ...
    for user in public_users:
        ...
        if user_usage:
            ...
            avg_electricity = sum(record.electricity_kwh for record in user_usage) / len(user_usage)
            avg_gas = sum(record.gas_cubic_m for record in user_usage) / len(user_usage)
            avg_carbon = sum(record.carbon_footprint or 0 for record in user_usage) / len(user_usage)
            ...
            global_data.append({
                ...
                'Avg Water (gal)': int(avg_water),
                'Avg Electricity (kWh)': int(avg_electricity),
                'Avg Gas (m³)': int(avg_gas),
                'Avg Carbon Footprint': round(float(avg_carbon), 1),
    ...
    if global_data:
        ...
        with col1:
            st.metric("Total Public Users", len(public_users))
        
        with col2:
            class_a_count = len([user for user in public_users if user.environmental_class == 'A'])
        ...
        try:
            if not global_df.empty and 'Environmental Class' in global_df.columns:
                ...
                if not class_counts.empty and len(class_counts) > 0:
                    ...
                    st.info("No environmental class data available for visualization.")
            else:
                st.info("No environmental class data available for visualization.")
        except Exception as e:
            st.warning("Unable to generate environmental class distribution chart.")
        ...
        try:
            if not global_df.empty and len(global_df) > 1:
                ...
                if 'Environmental Class' in global_df.columns and 'Avg Water (gal)' in global_df.columns:
                    fig_water = px.box(
                        global_df,
                    ...
                    st.info("Insufficient water usage data for comparison chart.")
                
                # Electricity usage by environmental class
                if 'Environmental Class' in global_df.columns and 'Avg Electricity (kWh)' in global_df.columns:
                    ...
                    st.info("Insufficient electricity usage data for comparison chart.")
            else:
                st.info("Need more user data (at least 2 users) to generate comparison charts.")
        except Exception as e:
            st.warning("Unable to generate usage comparison charts at this time.")
        
        # Leaderboard section
        st.subheader("🏆 Environmental Leaderboard")
        
        try:
            if not global_df.empty and 'Environmental Class' in global_df.columns and 'Avg Carbon Footprint' in global_df.columns:
                # Top performers by environmental class
                class_a_users = global_df[global_df['Environmental Class'] == 'A']
                if not class_a_users.empty and len(class_a_users) > 0:
                    class_a_sorted = class_a_users.sort_values('Avg Carbon Footprint')
                    st.write("**🌟 Top Class A Performers (Lowest Carbon Footprint):**")
                    for i, (_, user) in enumerate(class_a_sorted.head(5).iterrows(), 1):
                        st.write(f"{i}. **{user['Username']}** ({user['Location']}) - {user['Avg Carbon Footprint']} kg CO₂/month")
                else:
                    st.info("No Class A users available for leaderboard yet.")
            else:
                st.info("Insufficient data for leaderboard generation.")
        except Exception as e:
            st.warning("Unable to generate leaderboard at this time.")
        ...
        with col1:
            try:
                if not global_df.empty and 'Avg Water (gal)' in global_df.columns:
                    ...
                    st.metric("💧 Water Conservation Leader", "No data", "N/A")
            except (IndexError, KeyError, ValueError):
                st.metric("💧 Water Conservation Leader", "Error", "N/A")
        
        with col2:
            try:
                if not global_df.empty and 'Avg Electricity (kWh)' in global_df.columns:
                    ...
                    st.metric("⚡ Energy Efficiency Leader", "No data", "N/A")
            except (IndexError, KeyError, ValueError):
                st.metric("⚡ Energy Efficiency Leader", "Error", "N/A")
        
        with col3:
            try:
                if not global_df.empty and 'Avg Carbon Footprint' in global_df.columns:
                    ...
                    st.metric("🌱 Carbon Footprint Leader", "No data", "N/A")
            except (IndexError, KeyError, ValueError):
                st.metric("🌱 Carbon Footprint Leader", "Error", "N/A")
        ...
        st.info("No usage data available from public users yet.")
